...
AntHills::AntHills(std::string filename) : Maps(filename) {
    ...
    for (unsigned int x = 0; x < NUM_OF_CITIES; x++) {//filling matrix with basic values
        ...
        for (unsigned int y = 0; y < NUM_OF_CITIES; y++) {
            ...
            if (x == 0) {
                distance += distanceMatrix[0][y];//filling with first distance we can get since we dont know what worst option could be
            }
        }
    }
    iteration = 0;
...
void AntHills::thePathFinder(unsigned int start) {
    ...
    float tmp;
    float pheromones;
    for (int i = 0; i < ANT_QUANTITY; i++) {//pushing here vector
...
float AntHills::getBestDistance() {
    return bestDistance;
}
